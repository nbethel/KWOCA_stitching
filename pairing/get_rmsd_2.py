...
trimers=glob('../trimer_1/selects/snap*.pdb')
trimers=[]
fin=open('logger1.log','r')
lline=fin.readline()
while len(lline)>0:
    trimers.append(lline.split()[-2])
    lline=fin.readline()
trimers=np.unique(trimers)
dimers=glob('../dimer_2/selects/snap*.pdb')
fout=open('logger2.log','w')
